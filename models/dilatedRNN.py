...
class DilatedRNN(nn.Module):
    ...
    def one_step(self, x_t, hidden, t, mask):
        ...
        for l in range(self.num_layers):
            ...
            prev_t = t - (2 ** l)
            if prev_t >= 0:
                h_prev = hidden[l][prev_t]
            else:
                h_prev = hidden[l][0]  
            hidden[l][t] = self.activation(x2h(last_input) + h2h(h_prev))
            last_input = hidden[l][t]
            
        logits = self.linear_layer(last_input)  # (B, vocab)
        logits = logits + mask
    ...
    def forward(self, samples):
        ...
        hidden = [
            [torch.zeros(size=(B, self.units), device=self.device, dtype=torch.float64) for _ in range(self.seq_size)]
