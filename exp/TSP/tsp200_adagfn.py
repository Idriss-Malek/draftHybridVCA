...
from neighborVCA.algos import adaptive_gflownet
...
def run_instance(
    ...
    adagfn_cfg = master_config["adagfn"]
    train_cfg = master_config["training"]
    warmup_steps = int(train_cfg.get("n_warmup", 0))
    anneal_steps = int(train_cfg.get("n_anneal", 0))
    total_steps = warmup_steps + anneal_steps if (warmup_steps + anneal_steps) > 0 else int(adagfn_cfg.get("n_steps", 0))

    start = time.perf_counter()
    result = adaptive_gflownet(
        env,
        model,
        optimizer=optimizer,
        config=adagfn_cfg,
        n_steps=total_steps if total_steps > 0 else int(adagfn_cfg["n_steps"]),
        batch_size=int(train_cfg["batch_size"]),
        est_batch_size=adagfn_cfg.get("est_batch_size"),
    ...
    temp_hist = result["temperature"].tolist()
    delta_hist = result["delta"].tolist()
    ...
    with open(metrics_path, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["step", "temperature", "deltaT", "loss", "mean_energy", "min_energy", "max_energy", "var_energy"])
        for idx, (temp, delta, loss, mean_e, min_e, max_e, var_e) in enumerate(
            zip(temp_hist, delta_hist, loss_hist, energy_mean_hist, energy_min_hist, energy_max_hist, energy_var_hist)
        ):
            writer.writerow([idx, temp, delta, loss, mean_e, min_e, max_e, var_e])
...
def main(args: argparse.Namespace) -> None:
    ...
    results_dir = os.path.join(current_dir, "results_adagfn")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="TSP-200 adaptive GFlowNet experiment with extended logging.")
